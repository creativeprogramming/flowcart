 ...
 *
 */
class FlowcartModelCountries extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from('#__flowcart_countries AS c');

		// Filter by published state
		$published = $this->getState('filter.state');
		if (is_numeric($published))
		{
			$query->where('c.active = ' . (int) $published);
		}
		elseif ($published === '')
		{
			$query->where('(c.active IN (0, 1))');
		}
