 ...
 *
 */
class FlowcartModelStates extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from('#__flowcart_states AS s');

		// Filter by published state
		$published = $this->getState('filter.state');
		if (is_numeric($published))
		{
			$query->where('s.active = ' . (int) $published);
		}
		elseif ($published === '')
		{
			$query->where('(s.active IN (0, 1))');
		}
